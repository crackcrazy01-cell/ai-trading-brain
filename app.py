"""🚀 AI Trading Brain — Flask API Backend (pure Python, no numpy)"""
...
def index():
    return jsonify({
        "name": "AI Trading Brain API", "version": "2.0.0", "status": "running",
...
def health():
    return jsonify({
        "status": "healthy", "platform": "AI Trading Brain v2.0.0",
        "runtime": "Python/Flask", "brain_neurons": 50,
...
def get_portfolio():
    ...
    holdings = sum(
        pos["quantity"] * prices.get(pos["symbol"], {}).get("price", pos["entry_price"])
    ...
    return jsonify({
        ...
        "win_rate": round(wr, 1), "drawdown": round(dd, 2),
        "trades": tt, "wins": portfolio["wins"], "losses": portfolio["losses"],
...
def execute_trade():
    ...
    if direction == "buy":
        ...
        pos = {
            "symbol": symbol, "quantity": round(qty, 8),
            "entry_price": round(price, 4), "amount": round(amount, 2),
...
def brain_log():
    return jsonify({"generation": brain.generation, "log": brain.evolution_log[-50:]})

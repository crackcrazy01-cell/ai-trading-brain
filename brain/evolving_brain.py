...
class Neuron:
    def __init__(self, weights=None, bias=None):
        self.weights = weights if weights is not None else np.random.randn(20) * 0.5
        self.bias = bias if bias is not None else np.random.randn() * 0.5
        self.strength = 0.5
        self.wins = 0
        self.losses = 0
    ...
    def forward(self, inputs):
        ...
        self.last_signal = 1 / (1 + np.exp(-z))  # sigmoid
        return self.last_signal
    
    def learn(self, inputs, target, alpha=0.1):
        pred = self.forward(inputs)
        error = target - pred
        self.weights += alpha * error * np.array(inputs)
    ...
    @classmethod
    def from_dict(cls, d):
        n = cls(np.array(d['weights']), d['bias'])
...
class EvolvingBrain:
    ...
    def decide(self, features):
        """Run all neurons → vote BUY/SELL/HOLD"""
        buy_votes = 0
        sell_votes = 0
        total_confidence = 0
    ...
    def reinforce(self, features, reward):
        """Learn from trade result — positive reward strengthens, negative weakens"""
        alpha = 0.05
        for neuron in self.neurons:
    ...
    def evolve(self):
        """Kill bottom 40% neurons, breed top 60% with mutations"""
        ...
        for i in range(cutoff):
            ...
            child = Neuron(
                weights=parent.weights + np.random.randn(20) * 0.15,
                bias=parent.bias + np.random.randn() * 0.15
            )
    ...
    def get_health(self):
        ...
        avg_strength = np.mean([n.strength for n in self.neurons])
        return {
